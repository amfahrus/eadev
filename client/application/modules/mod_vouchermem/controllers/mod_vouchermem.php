			...
			switch ($nobukti) {
				...
				default:
					...
					/*foreach($jurnal as $key => $val){
						...
						if($val["dk"] == "D" && in_array($val["dperkir_id"], $dperkir_alokasi) && (strpos($val["nobukti"], 'RKS')) === false){
							$jurnal_alokasi[] = array(
								'tanggal' => $val["tanggal"],
						...
						if($val["dk"] == "K" && in_array($val["dperkir_id"], $dperkir_alokasi) && (strpos($val["nobukti"], 'RKS')) === false){
							$jurnal_alokasi[] = array(
								'tanggal' => $val["tanggal"],
								'nobukti' => "RK".$val["nobukti"],
								'dperkir_id' => 557,
