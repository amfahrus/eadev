...
<div class="content">
    <?= form_open("mod_group/group_add"); ?>
    <div class="row-fluid">
        <div class="span12">
            <div class="box">
                <div class="basic box_title"><h4><span>#</span> Group Akses Add</h4></div>
                <div class="basic box_content form-horizontal">
                    <div class="row-fluid">
                        <div class="control-group info">
                            <label class="control-label" for="nama_group">Nama Group</label>
                            <div class="controls">
                                <input class="span8 text" type="text" id="nama_group" name="nama_group" value="<?= set_value('nama_group'); ?>"/>
                            </div>
                        </div>
                    </div>
                    <div class="row-fluid">
                                ...
                                <ul id="tree1">
                                    ...
                                    if (isset($hak_akses)) {
                                        foreach ($hak_akses as $parent) {
                                            echo '<li><label class="checkbox"><input type="checkbox" class="checktree" name="hak_akses[]" value="' . $parent['row']['id_modules'] . '"> ' . $parent['row']['modules'] . '</label>';
                                            echo "<ul>";
                                            foreach ($parent['child'] as $module) {
                                                echo '<li><label class="checkbox"><input type="checkbox" class="checktree" name="hak_akses[]" value="' . $module['row']['id_modules'] . '"> ' . $module['row']['modules'] . '</label>';
                                                if (count($module['child'])) {
                                                    echo "<ul>";
                                                    foreach ($module['child'] as $submodule) {
                                                        echo '<li><label class="checkbox"><input type="checkbox" class="checktree" name="hak_akses[]" value="' . $submodule['row']['id_modules'] . '"> ' . $submodule['row']['modules'] . '</label>';

                                                        if (count($submodule['child'])) {
                                                            echo "<ul>";
                                                            foreach ($submodule['child'] as $submodulelast) {
                                                                echo '<li><label class="checkbox"><input type="checkbox" class="checktree" name="hak_akses[]" value="' . $submodulelast['row']['id_modules'] . '"> ' . $submodulelast['row']['modules'] . '</label>';
                                                            }
                                                            echo "</ul>";
                                                        }
                                                    }
