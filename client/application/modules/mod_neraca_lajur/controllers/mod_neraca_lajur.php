...
class mod_neraca_lajur extends CI_Controller {
    ...
    public function neraca_lajur_json($tipe = '') {
		...
		if($konsolidasi > 0){
			$query = $this->neraca_lajur_model->getKonsolidasi($konsolidasi,$periode_konsolidasi,$interval);
		} else {
    ...
    public function to_excel() {
		...
		if($konsolidasi > 0){
			$database = $this->neraca_lajur_model->getKonsolidasiForExcel($konsolidasi,$periode_konsolidasi,$interval);
			$namaproyek = $konsolidasi == 9999 ? 'Perusahaan' : $this->neraca_lajur_model->getUnitName($konsolidasi);
			switch ($interval) {
				case 1:
