...
class bukubesarrekanan_model extends CI_Model {

    protected $_table;
    protected $_countAll;
    protected $_arrayBukuBesarRekanan = array();
    ...
    public function getBukuBesarRekanan($limit, $offset, $sidx, $sord, $cari, $search) {
        if (!is_array($cari))
            $cari = array();
        ...
        $this->dataset_db->insert_logs(
										array(
											...
											'log_description' => 'User menampilkan laporan buku besar rekanan',
											'log_params' => json_encode($cari)
										)
									  );
        if ($search == "true") {
            $sql = "select * from getledgerrekanan(" . $cari["subunit_proyek"] . ", " . $cari["periode_awal"] . "," . $cari["periode_akhir"] . ", " . $cari["coa"] . ", '" . $cari["kode_rekanan"] . "')";
            $query = $this->db->query($sql);
    ...
    public function getBukuBesarRekananXls($cari) {
        if (!is_array($cari))
            $cari = array();
		$tmp_result = array();
		$sql = "select * from getledgerrekanan(" . $cari["subunit_proyek"] . ", " . $cari["periode_awal"] . "," . $cari["periode_akhir"] . ", '" . $cari["coa"] . "', '" . $cari["kode_rekanan"] . "')";
        ...
        $this->dataset_db->insert_logs(
										array(
											'log_username' => $this->session->userdata('ba_username'),
											'log_node' => $_SERVER['REMOTE_ADDR'],
											'log_description' => 'User mengunduh laporan buku besar rekanan',
											'log_params' => json_encode($cari)
										)
									  );
		foreach($data->result_array() as $row){
			$tmp_result[] = array(
						...
						'coa'			=> $row['gledger_coa'],
						'debet'			=> myFormatMoney($row['gledger_debet']),
						'kredit'		=> myFormatMoney($row['gledger_kredit']),
						'saldo'			=> myFormatMoney($row['gledger_saldo'])
    ...
    public function arraybukubesar() {
        $temp_result = array();
        foreach ($this->_arrayBukuBesarRekanan as $value) {
...
//                'text' => $row['period_name']
	...
	public function getbukubantu($proyek, $dperkir_id) {
        $sql = "SELECT a.bukubantu_kdrekanan, b.kode_rekanan||' - '||b.nama_rekanan as rekanan 
			    FROM tbl_bukubantu a
				LEFT JOIN tbl_rekanan b on a.bukubantu_kdrekanan = b.kode_rekanan AND a.bukubantu_id_proyek = b.id_proyek
				WHERE a.bukubantu_id_proyek = $proyek AND a.bukubantu_dperkir_id = $dperkir_id
				ORDER BY a.bukubantu_kdrekanan ASC";
        $query = $this->db->query($sql);
        $temp_result = array();
        foreach ($query->result_array() as $row) {
            $temp_result[] = array(
                //'image' => "",
                'description' => $row['rekanan'],
//                'description' => $row['period_range'],
                'value' => $row['bukubantu_kdrekanan'],
                //'text' => ""
//                'text' => $row['period_name']
            );
        }
...
a.id_proyek,
 a.kdperkiraan";
    ...
    public function getRekananName($kode){
		$this->db->select('nama_rekanan');
