...
class rpt_bukubesarrekanan extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('bukubesarrekanan_model');
    ...
    public function index() {
        ...
        $this->toolbar->addLink("", "btn tooltips", "#", "bukubesarrekanan_export_xls", "cus-page-white-excel", "Export Data Ke Excel", "tooltip", "right");
        $this->toolbar->addLink("", "btn tooltips", "#", "bukubesarrekanan_export_pdf", "cus-page-white-acrobat", "Export Data Ke PDF", "tooltip", "right");
        ...
        $DataModel = array(
            ...
            array(
                'text' => 'Kode Rekanan',
                'value' => 'text:Kode_Rekanan',
                'type' => 'custom',
                'callBack' => 'getbukubantu',
                'ops' => array("=")
            )
        );
        ...
        $data['getperiod'] = $this->bukubesarrekanan_model->getPeriod($this->_userconfig["kolom2"]);
    ...
    public function getjsonperiod() {
        ...
        $period = $this->bukubesarrekanan_model->getperiod($proyek);
	...
	public function getjsondperkir() {
        $userconfig = $this->dataset_db->getUserconfig($this->session->userdata('ba_user_id'));
        $proyek = $userconfig["kolom2"];
        $dperkir = $this->bukubesarrekanan_model->getdperkir($proyek);
        echo json_encode($dperkir);
    }
	
	public function getjsonbukubantu() {
        $userconfig = $this->dataset_db->getUserconfig($this->session->userdata('ba_user_id'));
        $proyek = $userconfig["kolom2"];
        $dperkir_id = $this->input->post('dperkir_id');
        $dperkir = $this->bukubesarrekanan_model->getbukubantu($proyek, $dperkir_id);
        echo json_encode($dperkir);
    }
    ...
    public function jsonBukuBesar() {
        ...
        $data = $this->bukubesarrekanan_model->getBukuBesarRekanan($limit, $offset, $sidx, $sord, $cari, $search);
...
//                ($row['gledger_kredit'] != "") ? myFormatMoney((int) $row['gledger_kredit']) : ""
    ...
    public function excel_bukubesar() {
        $data = $this->bukubesarrekanan_model->getBukuBesarRekanan(0, 100, 1, 1, 1, 1);
    }
    ...
    public function to_excel() {
		...
		$userconfig = $this->dataset_db->getUserconfig($this->session->userdata('ba_user_id'));
		$proyek = $this->bukubesarrekanan_model->getProyekName($userconfig["kolom2"]);
				 ...
				 if (!empty($row['cols']) AND !empty($row['ops']) AND !empty($row['vals'])) {
                    ...
                    switch ($fields[1]) {
                        case "Periode_Awal":
							$temp .= 'Periode Awal ';
							$temp .= $row['ops'].' ';
							$temp .= $this->bukubesarrekanan_model->getPeriodName($row['vals']);
                        ...
                        case "Kode_Rekanan":
                            $temp .= 'Rekanan';
							$temp .= $row['ops'].' ';
							$temp .= $this->bukubesarrekanan_model->getRekananName($row['vals']);
							$param["kode_rekanan"] = $row['vals'];
                            break;
                    }
		...
		if ($filter_count > 0) {
			$filtering = $filter_count + 4;
		}
		
        $database = $this->bukubesarrekanan_model->getBukuBesarRekananXls($param);
		
        if (count($database) > 0) {
            //$result = $database->result_array();
            //$last_line = $database->num_rows() + $filtering + 5;
            $last_line = count($database) + $filtering + 6;
        }
        ...
        $excel->filename = "BukuBesarRekanan.xls";
		...
		$excel->titleSheet('Buku Besar Rekanan')->startSheet();
    ...
    public function excel_test() {
        ...
        $database = $this->bukubesarrekanan_model->getBukuBesar(0, 100, 1, 1, 1, 1);
