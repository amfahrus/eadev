...
class mod_proyek extends CI_Controller {
    ...
    public function popup_proyek() {
        $data['content'] = $this->load->view('popup_proyek', "", true);
        $this->load->vars($data);
        $this->load->view('default_picker');
    }
    
    public function autocomplete_proyek() {
        $param = !empty($_GET['term']) ? $_GET['term'] : '';
        $query = $this->proyek_model->autocomplete($param);
        echo json_encode($query);
    }
    
	public function get_proyek() {

        $dt['id_proyek'] = $this->input->post('item');
        $json['json'] = $this->proyek_model->getPicker($dt);
        $this->load->view('template/ajax', $json);
    }
    ...
    public function popup_json() {
        ...
        if (strtolower($search) == "true") {
            $kode_proyek = isset($_GET['kodeproyek']) ? $_GET['kodeproyek'] : '';
            $nama_proyek = isset($_GET['namaproyek']) ? $_GET['namaproyek'] : '';
            $cari = array();
            $cari["kode_proyek"] = $kode_proyek;
            $cari["nama_proyek"] = $nama_proyek;
        } else {
            $cari = "";
        }
        ...
        $query = $this->proyek_model->PopupGetAll($limit, $offset, $sidx, $sord, $cari, $search);
        $count = $this->proyek_model->PopupCountAll();
        ...
        foreach ($query as $row) {
            $responce['rows'][$i]['id'] = $row['id_proyek'];
            $responce['rows'][$i]['cell'] = array('', $row['id_proyek'], $row['kode_proyek'], $row['nama_proyek']);
            $i++;
        }
        echo json_encode($responce);
    }
