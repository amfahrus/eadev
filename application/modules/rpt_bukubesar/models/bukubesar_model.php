...
class bukubesar_model extends CI_Model {
    ...
    public function getBukuBesar($limit, $offset, $sidx, $sord, $cari, $search) {
        ...
        $this->dataset_db->insert_logs(
										array(
											...
											'log_description' => 'User menampilkan laporan buku besar',
    ...
    public function getBukuBesarXls($cari) {
        if (!is_array($cari))
            $cari = array();
		$tmp_result = array();
		$sql = "select * from getgledger(" . $cari["subunit_proyek"] . ", " . $cari["periode_awal"] . "," . $cari["periode_akhir"] . ", '" . $cari["coa"] . "') --order by gledger_tanggal asc";
        ...
        $this->dataset_db->insert_logs(
										array(
											'log_username' => $this->session->userdata('ba_username'),
											'log_node' => $_SERVER['REMOTE_ADDR'],
											'log_description' => 'User mengunduh laporan buku besar',
											'log_params' => json_encode($cari)
										)
									  );
		...
		foreach($data->result_array() as $row){
			$tmp_result[] = array(
						...
						'no_bukti'		=> $row['gledger_no_dokumen'],
...
a.id_proyek,
 a.kdperkiraan";
    ...
    public function getDperkirName($id) {

        $this->db->select('nmperkiraan');
        $this->db->from('kodeperkiraan_v');
        $this->db->where('dperkir_id', $id);
        $query = $this->db->get();

        $result = '';
        foreach ($query->result_array() as $row) {
            $result = $row['nmperkiraan'];
        }
        return $result;
    }
}
